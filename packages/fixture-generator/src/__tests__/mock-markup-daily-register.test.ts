...
describe("get markup", () => {
  ...
  it("should be able to generate briefing tile of markup", () => {
    const mockDailyRegisterMarkup = new MockDailyRegisterMarkup()
      .addBriefing()
      .get();
    expect(mockDailyRegisterMarkup).toMatchObject([
      { name: "paragraph" },
      { name: "paragraph" },
    ]);
  });
  ...
  it("should generate large markup shapes", () => {
    expect(
      new MockDailyRegisterMarkup().addBirthdaysToday(5).get(),
    ).toMatchObject([
      { name: "paragraph" },
      { name: "paragraph" },
      { name: "paragraph" },
